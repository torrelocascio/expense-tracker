...
export class ExpenseService{
  ...
  getExpenses(){
    return this.http.get('http://git.heroku.com/expense-tracker-torrelocascio.git/expense')
      .map((response:any)=> {
...
updateExpense(expense:Expense){
  const body = JSON.stringify(expense)
  const headers = new Headers({'Content-Type': 'application/json'})
 return this.http.patch('http://git.heroku.com/expense-tracker-torrelocascio.git/expense/'+ expense.id , body , {headers: headers})
...
deleteExpense(expense: Expense){
  ...
  return this.http.delete('http://git.heroku.com/expense-tracker-torrelocascio.git/'+ expense.id)
  .map((response: Response) => response.json())
  // .catch((error: Response) => {
  //   this.errorService.handleError(error.json())
  //  return Observable.throw(error.json())
