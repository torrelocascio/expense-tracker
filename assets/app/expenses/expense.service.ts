...
export class ExpenseService{
  ...
  addExpense(expense: Expense, project: Project){
   ...
   return this.http.post('https://localhost:3000/expense/' + expense.project.id,body, {headers: headers})
          .map((response: any) => {
  ...
  getExpenses(){
    return this.http.get('https://localhost:3000/expense')
      .map((response:any)=> {
...
updateExpense(expense:Expense){
  const body = JSON.stringify(expense)
  const headers = new Headers({'Content-Type': 'application/json'})
 return this.http.patch('https://localhost:3000/expense/'+ expense.id , body , {headers: headers})
...
deleteExpense(expense: Expense){
  ...
  return this.http.delete('https://localhost:3000/expenses/'+ expense.id)
  .map((response: Response) => response.json())
  // .catch((error: Response) => {
  //   this.errorService.handleError(error.json())
  //  return Observable.throw(error.json())
