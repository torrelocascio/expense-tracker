...
export class ProjectService{
  ...
  addProject(project: Project, customer: Customer){
   ...
   return this.http.post('http://git.heroku.com/expense-tracker-torrelocascio.git/project/' + customer.id,body, {headers: headers})
          .map((response: any) => {
  ...
  getProjects(){
    return this.http.get('http://git.heroku.com/expense-tracker-torrelocascio.git/project')
...
updateProject(project:Project){
  const body = JSON.stringify(project)
  const headers = new Headers({'Content-Type': 'application/json'})
 return this.http.patch('http://git.heroku.com/expense-tracker-torrelocascio.git/'+ project.id , body , {headers: headers})
...
deleteProject(project: Project){
  ...
  console.log(project)
  return this.http.delete('http://git.heroku.com/expense-tracker-torrelocascio.git/'+ project.id)
  .map((response: Response) => response.json())
  .catch((error: Response) => {
    this.errorService.handleError(error.json())
   return Observable.throw(error.json())
